...
class Logminus extends REST_Controller {

    function __construct()
    {
        // Construct the parent class
        parent::__construct();
        $this->load->model('LogminusM');
    ...
    {
        $data = $this->LogminusM->getAllLogminus($this->get('id'));
        if($data){
            $response['status']=200;
    ...
    {
        $data = $this->LogminusM->getOneLogminus($this->get('id'));
        if($data){
            $response['status']=200;
            $response['error']=false;
    ...
    public function update_put()
    {
        $date = time();
        $id=$this->put('id');
        $temp = $this->LogminusM->getOneLogminus($id);
        ...
        $data = $this->LogminusM->updateLogminus($val,$id);
        if($data){
            $response['status']=200;
            $response['error']=false;
    ...
    public function add_post()
    {
        ...
        $data = $this->LogminusM->insertLogminus($val);
        if($data){
            $response['status']=200;
            $response['error']=false;
            $response['result']='Berhasil.';
    ...
    {
        $data = $this->LogminusM->deleteLogminus($this->delete('id'));
        if($temp){
            $response['status']=200;
            $response['error']=false;
            $response['result']='Berhasil.';
